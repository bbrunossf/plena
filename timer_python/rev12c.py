"""
VERSÃO DE TESTE COM ALTERAÇÃO DA INTERFACE (USANDO CUSTOMTKINTER)
FICOU FEIO, NÃO GOSTEI; NÃO USAR
"""
...
# Adicionar configuração do customtkinter após os imports
...
class AboutDialog:
    ...
    def setup_gui(self):
        ...
        self.dialog.rowconfigure((0,1,2,3), weight=1)
...
class ProjectTrackingWindow:
    ...
    def setup_gui(self):
        ...
        # Project selection
        project_frame = ctk.CTkFrame(self.window)
        project_frame.grid(row=0, column=0, pady=10, padx=20, sticky="ew")
        project_frame.columnconfigure(0, weight=1)
        
        ctk.CTkLabel(project_frame, text="Projeto:", font=ctk.CTkFont(weight="bold")).grid(row=0, column=0, sticky="w", padx=10, pady=(10,5))
        ...
        self.project_dropdown.grid(row=1, column=0, sticky="ew", padx=10, pady=(0,10))
        self.project_dropdown.bind('<<ComboboxSelected>>', self.update_tasks)
        
        # Task selection
        task_frame = ctk.CTkFrame(self.window)
        task_frame.grid(row=1, column=0, pady=10, padx=20, sticky="ew")
        task_frame.columnconfigure(0, weight=1)
        
        ctk.CTkLabel(task_frame, text="Tarefa:", font=ctk.CTkFont(weight="bold")).grid(row=0, column=0, sticky="w", padx=10, pady=(10,5))
        
        self.task_var1 = tk.StringVar()
        self.task_dropdown1 = ctk.CTkComboBox(
            task_frame,
            variable=self.task_var1,
            state="readonly",
            width=250
        )
        self.task_dropdown1.grid(row=1, column=0, sticky="ew", padx=10, pady=(0,10))
        self.task_dropdown1.bind('<<ComboboxSelected>>', self.update_categoria)
        
        ctk.CTkLabel(task_frame, text="SubTarefa:", font=ctk.CTkFont(weight="bold")).grid(row=2, column=0, sticky="w", padx=10, pady=(5,5))
        
        self.task_var2 = tk.StringVar()
        self.task_dropdown2 = ctk.CTkComboBox(
            task_frame,
            variable=self.task_var2,
            state="readonly",
            width=250
        )
        self.task_dropdown2.grid(row=3, column=0, sticky="ew", padx=10, pady=(0,10))
        
        # Checkbox para hora extra
        extra_hour_frame = ctk.CTkFrame(self.window)
        extra_hour_frame.grid(row=2, column=0, pady=10, padx=20, sticky="ew")
        
        self.hora_extra_var = tk.BooleanVar()
        self.hora_extra_checkbox = ctk.CTkCheckBox(
            extra_hour_frame,
            text="Hora extra?",
            variable=self.hora_extra_var,
            font=ctk.CTkFont(size=12)
        )
        self.hora_extra_checkbox.grid(row=0, column=0, padx=10, pady=10, sticky="w")
        ...
        self.ok_button.grid(row=3, column=0, pady=20)
...
class CountdownTimer:
    ...
    def setup_gui(self):
        ...
        self.time_label.grid(row=0, column=0, pady=20, padx=20, sticky="ew")
        
        # Buttons frame
        button_frame = ctk.CTkFrame(self.root)
        button_frame.grid(row=1, column=0, pady=20, padx=20, sticky="ew")
        button_frame.columnconfigure((0,1), weight=1)
    ...
    def update_display(self):
        self.time_label.configure(text=str(self.remaining_time).split(".")[0])
        
       
        
    def timer_complete(self):
        self.pause_timer()
        self.time_label.configure(text="Fim!")
        self.root.bell()
        self.root.deiconify()
...
def main():    
    app = CountdownTimer()
    app.root.bind("<Map>", app.restore_window)
    app.root.mainloop()
